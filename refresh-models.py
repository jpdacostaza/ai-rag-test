        ...
        try:
            async with httpx.AsyncClient(
                ...
                if response.status_code == 200:
                    ...
                    logger.error(
                        f"Failed to get backend models: {response.status_code}"
                    )
                    return []
        except Exception as e:
        ...
        try:
            async with httpx.AsyncClient(
                ...
                response = await client.get(
                    f"{BACKEND_URL}/v1/models/verify/llama3.2:3b"
                )
                if response.status_code in [
                    200,
                    404,
                ]:  # 404 is ok, means model check was performed
                    logger.info("Backend model refresh triggered")
                    return True
                else:
                    logger.error(
                        f"Failed to trigger backend refresh: {response.status_code}"
                    )
                    return False
        except Exception as e:
