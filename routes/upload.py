...
async def upload_document_json(upload: DocumentUpload) -> Dict[str, Any]:
    ...
    try:
        ...
        log_service_status(
            "upload", 
            "info", 
            f"Document uploaded successfully: {chunks_stored} chunks for user {upload.user_id}"
        )
...
async def search_documents_json(search: DocumentSearch) -> Dict[str, Any]:
    ...
    try:
        ...
        log_service_status(
            "upload", 
            "info", 
            f"Search completed: {len(formatted_results)} results for user {search.user_id}"
        )
...
async def get_supported_formats() -> Dict[str, Any]:
    ...
    return {
        ...
        "limits": {
            "max_content_length": 1000000,  # 1MB for JSON content
