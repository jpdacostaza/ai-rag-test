...
class MemoryCacheHealthTester:
    ...
    def run_startup_tests(self) -> Dict[str, Any]:
        ...
        log_service_status(
            "HEALTH", "starting", "🧪 Running memory & cache startup tests"
        )
        ...
        if results["services_healthy"] == results["services_tested"]:
            ...
            log_service_status(
                "HEALTH",
                "ready",
                f"✅ All {results['services_healthy']} memory services healthy",
            )
            ...
            log_service_status(
                "HEALTH",
                "degraded",
                f"⚠️ {results['services_healthy']}/{results['services_tested']} memory services healthy",
            )
        else:
            ...
            log_service_status(
                "HEALTH",
                "failed",
                f"❌ Only {results['services_healthy']}/{results['services_tested']} memory services healthy",
            )
    ...
    def run_quick_health_check(self) -> Dict[str, Any]:
        ...
        # Determine overall status
        healthy_tests = sum(
            1 for test in results["tests"].values() if test.get("status") == "healthy"
        )
        total_tests = len(
            [
                t
                for t in results["tests"].values()
                if t.get("status") in ["healthy", "unhealthy"]
            ]
        )
    ...
    def _test_chat_history(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if length != len(test_messages):
                return {
                    "status": "unhealthy",
                    "error": f"Expected {len(test_messages)} messages, got {length}",
                }
            ...
            if len(retrieved_msgs) != len(test_messages):
                return {
                    "status": "unhealthy",
                    "error": "LRANGE returned wrong number of messages",
                }
            ...
            if trimmed_length != 2:
                return {
                    "status": "unhealthy",
                    "error": f"LTRIM failed, expected 2 messages, got {trimmed_length}",
                }
    ...
    def _test_chromadb_memory(self) -> Dict[str, Any]:
        ...
        try:
            log_service_status(
                "HEALTH", "testing", "Testing ChromaDB memory operations"
            )

            if not self.db_manager.chroma_collection:
                return {
                    "status": "unhealthy",
                    "error": "No ChromaDB collection available",
                }
            ...
            test_ids = [
                f"{self.test_prefix}:doc_{i}" for i in range(len(test_documents))
            ]
            test_metadata = [
                {"source": "health_test", "topic": f"topic_{i}"}
            ...
            self.db_manager.chroma_collection.add(
                documents=test_documents, ids=test_ids, metadatas=test_metadata
            )
            ...
            if count < len(test_documents):
                return {
                    "status": "unhealthy",
                    "error": f"Document count too low: {count}",
                }
            ...
            if len(results.get("ids", [])) != len(test_ids):
                return {
                    "status": "unhealthy",
                    "error": "GET operation returned wrong number of documents",
                }
            ...
            if self.db_manager.embedding_model:
                query_results = self.db_manager.chroma_collection.query(
                    query_texts=["artificial intelligence"], n_results=1
                )
                if not query_results.get("ids"):
                    return {
                        "status": "unhealthy",
                        "error": "QUERY operation returned no results",
                    }
    ...
    def _test_embeddings(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if np.any(np.isnan(embeddings)) or np.any(np.isinf(embeddings)):
                return {
                    "status": "unhealthy",
                    "error": "Invalid embedding values (NaN or Inf)",
                }
    ...
    def _test_cache_manager(self) -> Dict[str, Any]:
        ...
        try:
            ...
            set_result = self.cache_manager.set_with_validation(
                test_key, test_value, 60
            )
            if not set_result:
                return {
                    "status": "unhealthy",
                    "error": "Cache manager SET operation failed",
                }
            ...
            if retrieved_value != test_value:
                return {
                    "status": "unhealthy",
                    "error": "Cache manager GET operation failed",
                }
            operations_tested.append("get_with_validation")

            # Test JSON rejection
            json_value = '{"test": "json_data"}'
            json_result = self.cache_manager.set_with_validation(
                f"{test_key}_json", json_value, 60
            )
            if json_result:
                return {
                    "status": "unhealthy",
                    "error": "Cache manager should reject JSON format",
                }
            ...
            if redis_client:
                ...
                while True:
                    cursor, keys = redis_client.scan(
                        cursor, match=f"{self.test_prefix}*", count=1000
                    )
                ...
                if test_keys:
                    ...
                    log_service_status(
                        "HEALTH",
                        "ready",
                        f"Cleaned up {len(test_keys)} Redis test keys",
                    )
                ...
                try:
                    ...
                    if results.get("ids"):
                        ...
                        log_service_status(
                            "HEALTH",
                            "ready",
                            f"Cleaned up {len(results['ids'])} ChromaDB test documents",
                        )
                except Exception as e:
                    log_service_status(
                        "HEALTH", "warning", f"ChromaDB cleanup failed: {e}"
                    )
