...
# from pipelines_routes import router as pipelines_router  # Implemented directly in main.py
...
# Test endpoints directly in main.py for debugging
@app.post("/test/inlet")
...
# Include v1 pipelines router
app.include_router(pipelines_v1_router)
# Include pipelines router - implemented directly in main.py
    ...
    try:
        ...
        if not tool_used:
            ...
            async def llm_query():
                ...
                query_emb = get_embedding(db_manager, user_message)
...
async def retrieve_memory_for_pipeline(
    ...
    try:
        ...
        if query_embedding is not None:
            memories = retrieve_user_memory(db_manager, user_id, query_embedding, n_results=limit)
...
async def process_interaction_for_pipeline(
    ...
    try:        # Extract request data
        ...
        user_message = request.get("user_message", "") or request.get("message", "")  # Support both formats
...
async def list_pipelines():
    ...
    pipelines = [{
        ...
        "type": "filter",
        "description": "Persistent memory and learning pipeline for cross-chat context",
        "author": "Backend Team",
        "version": "1.0.0"
    }]
...
# Basic Memory Endpoints for Pipeline Integration
@app.post("/learn")
async def learn_endpoint(
    request: dict,
    api_key: str = Depends(verify_api_key)
):
    """Simple learning endpoint for pipeline integration"""
    try:
        ...
        log_service_status("LEARNING", "info", f"Learning request for user {user_id}")
        
        # Store the memory using the document indexing system
        timestamp = datetime.now().isoformat()
        doc_id = f"memory_{user_id}_{int(datetime.now().timestamp())}"
        doc_name = f"Memory: {context}"
        ...
        if success:
            log_service_status("LEARNING", "ready", f"Memory stored for user {user_id}")
            return {"status": "success", "message": "Memory stored successfully"}
        else:
            log_service_status("LEARNING", "warning", f"Failed to store memory for user {user_id}")
            return {"status": "error", "message": "Failed to store memory"}
            
    except Exception as e:
        log_error(e, "learn_endpoint", request.get("user_id", "unknown"), "pipeline")
...
async def get_memories_endpoint(
    ...
    try:
        log_service_status("MEMORY_RETRIEVAL", "info", f"Memory retrieval request for user {user_id}")
          # Create a general query embedding to retrieve memories
        query_text = f"memories for user {user_id}"
        query_embedding = get_embedding(db_manager, query_text)
        
        if not query_embedding:
            log_service_status("MEMORY_RETRIEVAL", "warning", f"Could not generate embedding for user {user_id}")
            return []
        
        # Retrieve memories using the existing database function
        memories = retrieve_user_memory(db_manager, user_id, query_embedding, n_results=limit)
        
        # Format the memories for pipeline use
        formatted_memories = []
        for memory in memories:
            formatted_memories.append({
                ...
                "context": memory.get("metadata", {}).get("context", "general")
            })
        
        log_service_status("MEMORY_RETRIEVAL", "ready", f"Retrieved {len(formatted_memories)} memories for user {user_id}")
        return formatted_memories
        
    except Exception as e:
        log_error(e, "get_memories_endpoint", user_id, "pipeline")
