...
class BackendCleanup:
    ...
    def optimize_requirements(self):
        ...
        if os.path.exists(req_file):
            ...
            if duplicates:
                logger.warning(
                    f"Found duplicate packages in requirements.txt: {duplicates}"
                )
            else:
            ...
            if os.path.exists(config_file):
                try:
                    with open(config_file, "r", encoding="utf-8") as f:
                        content = f.read()
                        if (
                            "OLLAMA_URL=" in content
                            and "OLLAMA_BASE_URL=" not in content
                        ):
                            inconsistencies.append(
                                f"{config_file} uses OLLAMA_URL instead of OLLAMA_BASE_URL"
                            )
    ...
    def run_cleanup(self):
        ...
        logger.info(
            f"  - Directories cleaned: {self.cleanup_stats['directories_cleaned']}"
        )
        logger.info(
            f"  - Space saved: {self.cleanup_stats['bytes_saved'] / 1024:.2f} KB"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Comprehensive backend project cleanup"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
