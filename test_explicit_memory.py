...
def test_explicit_memory():
    ...
    if response.status_code == 200:
        ...
        pizza_memories = [m for m in memories if "pizza" in m.get("content", "").lower()]
        print(f"   Found {len(pizza_memories)} pizza memories:")
        for memory in pizza_memories:
    ...
    if response.status_code == 200:
        ...
        print(f"   ✅ Forget successful: deleted {deleted_count} memories")
    else:
    ...
    # Test 4: Query again to see if memories were deleted
    ...
    if response.status_code == 200:
        ...
        memories = result.get("memories", [])
        pizza_memories = [m for m in memories if "pizza" in m.get("content", "").lower()]
        print(f"   Found {len(pizza_memories)} pizza memories after deletion")
        
        if len(pizza_memories) == 0:
