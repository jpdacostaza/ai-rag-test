...
class Pipeline:
    ...
    async def on_shutdown(self):
        ...
        self.log(
            f"📊 Final stats - Requests: {self.total_requests}, Memory hits: {self.memory_hits}, Learning stored: {self.learning_stored}"
        )
    ...
    async def on_valves_updated(self):
        ...
        if self.valves.debug_logging:
            self.log(
                f"🔧 New config - Memory: {self.valves.enable_memory_retrieval}, Learning: {self.valves.enable_adaptive_learning}"
            )
    ...
    async def check_backend_health(self) -> bool:
        ...
        try:
            async with httpx.AsyncClient(timeout=self.valves.timeout_seconds) as client:
                response = await client.get(f"{self.valves.backend_url}/api/pipeline/status")
    ...
    async def store_interaction(self, user_id: str, user_message: str, assistant_response: str, response_time: float):
        ...
        try:
            ...
            if self.valves.async_processing:
                ...
                asyncio.create_task(
                    self._store_interaction_async(user_id, user_message, assistant_response, response_time)
                )
            else:
    ...
    async def _store_interaction_async(
        self, user_id: str, user_message: str, assistant_response: str, response_time: float
    ):
