...
def initialize_cache_management():
    ...
    try:
        ...
        current_prompt = "You are a helpful assistant. Use the following memory and chat history to answer. Always \
            respond with plain text only - never use JSON formatting, structured responses, or any special formatting. Just provide direct, natural language answers."
        ...
        log_service_status(
            "CACHE_INIT",
            "ready",
            "Cache management initialized. Version: {stats.get('version', 'unknown')}, Total keys: \
                {stats.get('total_keys', 0)}",
        )

        return True
    except Exception:
        log_service_status("CACHE_INIT", "failed", "Cache initialization failed: {e}")
        return False
