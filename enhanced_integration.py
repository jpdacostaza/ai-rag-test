...
from enhanced_document_processing import enhanced_chunker
...
async def upload_document_advanced(
    ...
    try:
        ...
        log_service_status(
            ...
            f"Processed {file.filename or 'unknown_file'}: {success_count}/{len(processed_chunks)} chunks stored in \
...
async def get_user_insights(user_id: str):
    ...
    try:
        ...
        return JSONResponse(status_code=200, content={"success": True, "data": insights})
...
async def get_chunking_strategies():
    ...
    return {
        "strategies": [
            {"name": strategy.value, "description": _get_strategy_description(strategy)}
            for strategy in ChunkingStrategy
        ],
        "document_types": [
            {"name": doc_type.value, "description": _get_document_type_description(doc_type)}
...
async def enhanced_chat_endpoint(
    user_id: str = Form(...), message: str = Form(...), conversation_id: Optional[str] = Form(None)
):
