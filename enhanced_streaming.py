...
class EventDispatcher:
    ...
    def register_listener(
        event_type: str, callback: Union[Callable[[StreamEvent], None], Callable[[StreamEvent], Awaitable[None]]]
    ) -> None:
    ...
    async def dispatch_custom_event(
        ...
        event = StreamEvent(
            event_type=event_type, data=data, timestamp=time.time(), session_id=session_id, metadata=metadata or {}
        )
...
async def enhanced_stream_wrapper(
    ...
    await EventDispatcher.dispatch_custom_event(
        "stream_started",
        {"session_id": session_id, "model_name": model_name, "monitor_enabled": monitor_enabled},
...
async def robust_llm_call(llm_function: Callable, *args, session_id: Optional[str] = None, **kwargs) -> Any:
    ...
    try:
        if session_id:
            await EventDispatcher.dispatch_custom_event(
                "llm_call_attempt", {"session_id": session_id, "function": llm_function.__name__}, session_id=session_id
            )
        ...
        log_service_status("STREAMING", "error", f"Error in LLM call {llm_function.__name__}: {e}")
        if session_id:
            await EventDispatcher.dispatch_custom_event(
                "llm_call_failed",
                {
...
class StreamingResponseWithEvents(StreamingResponse):
    ...
    def __init__(
        ...
        enhanced_content = enhanced_stream_wrapper(content, session_id=session_id, monitor_enabled=monitor_enabled)

        super().__init__(
            content=enhanced_content, status_code=status_code, headers=final_headers, background=background
        )
...
class UsageTracker:
    ...
    def __new__(cls):
        """TODO: Add proper docstring for __new__."""
