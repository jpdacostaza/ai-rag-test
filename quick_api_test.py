...
def test_api():
    ...
    try:
        chat_data = {"user_id": "quick_test_user", "message": "Hello, what is 2+2?"}
        response = requests.post(
            f"{base_url}/chat", headers=headers, json=chat_data, timeout=30
        )
        print(f"   Status: {response.status_code}")
        if response.status_code == 200:
            data = response.json()
            ...
            print(
                f"   ✅ Chat endpoint passed - Response length: {len(response_text)} chars"
            )
