...
def test_direct_document_storage():
    ...
    data = {"user_id": TEST_USER_ID, "name": "Test Memory Document"}

    response = requests.post(
        f"{BACKEND_URL}/upload", files=files, data=data, headers={"Authorization": f"Bearer {API_KEY}"}, timeout=30
    )
...
def test_direct_memory_retrieval():
    ...
    # Retrieve memories
    query_data = {"user_id": TEST_USER_ID, "query": "What is my name and job?", "limit": 5}
