...
def test_memory_storage():
    ...
    try:
        ...
        response = requests.post(f"{BACKEND_URL}/api/learning/process_interaction", json=data, headers=headers, timeout=10)
...
def test_pipeline_inlet():
    ...
    try:
        data = {
            "body": {
                "messages": [
                    {
                        "role": "user",
                        "content": "Hi, what do you remember about me?"
                    }
                ]
            },
        ...
        headers = {"Authorization": f"Bearer {API_KEY}", "Content-Type": "application/json"}
        response = requests.post(f"{BACKEND_URL}/pipelines/memory_pipeline/inlet", json=data, headers=headers, timeout=10)
        ...
        if response.status_code == 200:
            ...
            memory_injected = any('remember' in msg.get('content', '').lower() or 
                                'name' in msg.get('content', '').lower() or 
                                'python' in msg.get('content', '').lower() 
                                for msg in messages)
...
def test_pipeline_outlet():
    ...
    try:
        data = {
            "body": {
                "messages": [
                    {
                        "role": "user",
                        "content": "I also enjoy machine learning and AI development"
                    },
                    {
                        "role": "assistant",
        ...
        headers = {"Authorization": f"Bearer {API_KEY}", "Content-Type": "application/json"}
        response = requests.post(f"{BACKEND_URL}/pipelines/memory_pipeline/outlet", json=data, headers=headers, timeout=10)
