...
class ComprehensiveRealLifeTest:
        ...
        while time.time() - start_time < timeout:
            try:
                if url.startswith("redis://"):
                    ...
                    print(
                        "   Assuming Redis is available (container status shows healthy)"
                    )
                    return True
                else:
        ...
        try:
            ...
            if all_healthy:
                self.log_test_result(
                    "Docker Container Health", True, "All containers healthy"
                )
            else:
                failed_services = [
            ...
            self.log_test_result(
                "Docker Container Health", False, f"Exception: {str(e)}"
            )
            return False
        ...
        try:
            ...
            try:
                detailed_response = requests.get(
                    f"{self.base_url}/health/detailed", timeout=30
                )
            ...
            self.log_test_result(
                "Startup Memory Health Integration", False, f"Exception: {str(e)}"
            )
            return False
    ...
    def test_cache_manager_integration(self) -> bool:
        ...
        try:
            ...
            cache_hit_detected = (
                len(response_times) >= 3 and response_times[2] < response_times[0] * 0.8
            )
            ...
            try:
                ...
                if health_response.status_code == 200:
                    ...
                    current_cache_keys = health_data.get("cache", {}).get(
                        "total_keys", 0
                    )
                    self.test_data["cache_manager"][
                        "final_cache_keys"
            ...
            success = all(
                cb["status"] == "success" for cb in cache_behavior[:2]
            )  # At least first 2 should succeed
            ...
            self.log_test_result(
                "Cache Manager Integration", False, f"Exception: {str(e)}"
            )
            return False
    ...
    def test_ai_tools_integration(self) -> bool:
        ...
        try:
            ...
            success = (
                successful_tools >= len(tool_tests) // 2
    ...
    def test_document_upload_and_rag(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                upload_details = (
                    upload_response.json() if upload_success else upload_response.text
                )
                ...
                if upload_success:
                    ...
                    rag_query = (
                        "What year was the company founded and where is it located?"
                    )
            ...
            self.test_data["document_rag"] = {
                ...
                "rag_response": (
                    rag_content[:200] if "rag_content" in locals() else "No response"
                ),
            }
            ...
            self.log_test_result(
                "Document Upload and RAG", False, f"Exception: {str(e)}"
            )
            return False
    ...
    def test_error_handling_and_feedback(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                error_tests.append(
                    {"test": "Invalid API Key", "error": str(e), "handled": False}
                )
            ...
            try:
                ...
                error_tests.append(
                    {"test": "Malformed Request", "error": str(e), "handled": False}
                )
            ...
            try:
                ...
                error_tests.append(
                    {"test": "Health Endpoint", "error": str(e), "handled": False}
                )

            self.test_data["error_handling"] = error_tests

            handled_errors = sum(
                1 for test in error_tests if test.get("handled", False)
            )
            success = (
                handled_errors >= len(error_tests) * 0.7
            )  # 70% should be handled properly
            ...
            self.log_test_result(
                "Error Handling and Feedback", False, f"Exception: {str(e)}"
            )
            return False
        ...
        try:
            ...
            try:
                storage_response = requests.get(
                    f"{self.base_url}/health/storage", timeout=10
                )
            ...
            try:
                chroma_response = requests.get(
                    f"{self.chroma_url}/api/v1/heartbeat", timeout=5
                )
            ...
            self.log_test_result(
                "Storage and Persistence", False, f"Exception: {str(e)}"
            )
            return False
    ...
    def test_adaptive_learning_system(self) -> bool:
        ...
        try:
            ...
            if chat_response.status_code == 200:
                ...
                try:
                    stats_response = requests.get(
                        f"{self.base_url}/adaptive/stats", timeout=10
                    )
            ...
            basic_success = any(
                test.get("success") and "Basic Chat" in test.get("test", "")
                for test in learning_tests
            )
            success = basic_success

            successful_tests = sum(
                1 for test in learning_tests if test.get("success", False)
            )
            details = (
                f"Learning components working: {successful_tests}/{len(learning_tests)}"
            )
            ...
            self.log_test_result(
                "Adaptive Learning System", False, f"Exception: {str(e)}"
            )
            return False
        ...
        try:
            ...
            try:
                ...
                scenario_steps.append(
                    {"step": "Document Upload", "success": upload_success}
                )
            ...
            if upload_success:
                rag_query = (
                    "When was TechCorp founded and how many employees do they have?"
                )
                chat_payload = {
                    "messages": [{"role": "user", "content": rag_query}],
                    "model": "gpt-4o-mini",
                ...
                if rag_success:
                    ...
                    scenario_steps.append(
                        {"step": "RAG Quality", "success": rag_quality}
                    )
            ...
            if tool_success:
                ...
                cache_hit_likely = (
                    cache_time < 1.0
                )  # Less than 1 second suggests cache hit
                scenario_steps.append(
                    {
                        "step": "Cache Test",
                        "success": cache_success and cache_hit_likely,
                    }
                )
            ...
            self.log_test_result(
                "Comprehensive Integration Scenario", False, f"Exception: {str(e)}"
            )
            return False
    ...
    def test_persona_and_configuration(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                is_structured = any(
                    char in content for char in ["1.", "2.", "-", "*"]
                )  # Some structure
            ...
            try:
                ...
                config_success = config_response.status_code in [
                    200,
                    404,
                ]  # 404 is acceptable
                persona_tests.append(
            ...
            personality_success = any(
                test.get("success") and "Personality" in test.get("test", "")
            ...
            self.log_test_result(
                "Persona and Configuration", personality_success, details
            )
            return personality_success

        except Exception as e:
            self.log_test_result(
                "Persona and Configuration", False, f"Exception: {str(e)}"
            )
            return False
    ...
    def generate_comprehensive_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "categories": {
                ...
                "core": {
                    ...
                    "passed": sum(
                        1
                        for test in core_functionality
                        if self.test_results.get(test, {}).get("success", False)
                    ),
        ...
        for category, data in report["categories"].items():
            ...
            status_emoji = (
                "✅" if cat_pass_rate >= 80 else "⚠️" if cat_pass_rate >= 60 else "❌"
            )
            print(
                f"   {category.capitalize()}: {data['passed']}/{data['total']} ({cat_pass_rate:.1f}%) {status_emoji}"
            )
    ...
    def generate_recommendations(self, report: Dict[str, Any]) -> List[str]:
        ...
        if "AI Tools Integration" in failed_tests:
            recommendations.append(
                "Verify API keys for weather service and tool implementations"
            )
        ...
        if "Storage and Persistence" in failed_tests:
            recommendations.append(
                "Validate storage directory permissions and persistence setup"
            )

        # Performance recommendations
        cache_data = self.test_data.get("cache_manager", {})
        if cache_data.get("cache_hit_detected") is False:
            recommendations.append(
        ...
        if report["test_summary"]["pass_rate"] >= 80:
            recommendations.append(
                "✨ Consider implementing monitoring and alerting for production"
            )
            recommendations.append(
