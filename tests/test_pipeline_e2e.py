...
def simulate_openwebui_workflow():
    ...
    if response.status_code == 200:
        ...
        has_memory_context = any(
            'alice' in msg.get('content', '').lower() or 
            'python' in msg.get('content', '').lower() or
            'developer' in msg.get('content', '').lower() or
            'san francisco' in msg.get('content', '').lower()
            for msg in messages
        )
...
def test_cross_chat_memory():
    ...
    inlet_data = {
        "body": {
            "messages": [
                {
                    "role": "user",
                    "content": "Do you remember what I told you about my work?"
                }
            ]
        },
        "user": {"id": TEST_USER_ID}
    ...
    if response.status_code == 200:
        ...
        memory_found = any(
            ...
            'python' in msg.get('content', '').lower() or
            'developer' in msg.get('content', '').lower() or
            'ai projects' in msg.get('content', '').lower()
            for msg in messages
        )
