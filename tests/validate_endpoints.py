    ...
    try:
        ...
        for pattern in patterns:
            ...
            for method, path in matches:
                endpoints.append({"method": method.upper(), "path": path, "file": os.path.basename(filepath)})
    ...
    try:
        ...
        for imp in imports:
            ...
            if "." in imp:
                ...
                potential_paths = [f"{'/'.join(parts)}.py", f"{'/'.join(parts)}/__init__.py", f"{parts[0]}.py"]
            else:
            ...
            for path in potential_paths:
                if os.path.exists(path) or imp in [
                    "fastapi",
                    "pydantic",
                    "redis",
                    "httpx",
    ...
    for router_file in router_files:
        if check_file_exists(router_file):
            ...
            if not valid_imports:
                import_issues.append({"file": router_file, "issues": import_result})
        else:
