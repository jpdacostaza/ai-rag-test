        ...
        if scenario["pattern"] == "different_users_same_questions":
            ...
            for question in common_questions[:4]:
                for user in users[:3]:
                    ...
                    # Check cache
                    cached = get_cache(
                        db_manager, cache_key, user, f"req_{user}_{question[:10]}"
                    )

                    if not cached:
                        # Simulate response generation and caching
                        response = (
                            f"Detailed response about {question} - generated for {user}"
                        )
                        set_cache(
                            db_manager,
                            cache_key,
            ...
            for user in users[:3]:
                for question in common_questions[:2]:
                    ...
                    cached = get_cache(
                        db_manager, cache_key, user, f"repeat_{user}_{question[:10]}"
                    )
            ...
            for i in range(10):
                ...
                cache_key = (
                    f"chat:{user}:{hashlib.sha256(question.encode()).hexdigest()[:16]}"
                )
                ...
                if not cached:
                    ...
                    set_cache(
                        db_manager, cache_key, response, 600, user, f"mix_set_{i}"
                    )
...
def demonstrate_system_prompt_impact():
    ...
    print("📊 Cache state before prompt change:")
