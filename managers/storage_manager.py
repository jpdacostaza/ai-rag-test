...
class StorageManager:
    ...
    def ensure_storage_structure(cls) -> Dict[str, bool]:
        ...
        for name, config in cls.STORAGE_STRUCTURE.items():
            ...
            try:
                ...
                log_service_status(
                    "STORAGE", "error", f"Failed to create {name}: {str(e)}"
                )
                results[name] = False
    ...
    def validate_permissions(cls) -> Dict[str, bool]:
        ...
        if not base_path.exists():
            log_service_status(
                "STORAGE", "error", "Base storage directory does not exist"
            )
            return {"base": False}
...
def initialize_storage() -> bool:
    ...
    try:
        ...
        log_service_status(
            "STORAGE", "error", f"Storage initialization failed: {str(e)}"
        )
        return False
...
if __name__ == "__main__":
    ...
    for name, valid in permissions.items():
        ...
        print(
            f"{status} {name}: {'Write access OK' if valid else 'Write access FAILED'}"
        )
