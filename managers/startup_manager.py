...
class StartupManager:
    ...
    async def initialize_application(self):
        ...
        log_service_status(
            "STARTUP", "starting", "🚀 FastAPI LLM Backend startup initiated"
        )
        ...
        log_service_status(
            "STARTUP", "starting", "Initializing cache management and memory systems..."
        )
        ...
        log_service_status(
            "STARTUP", "starting", "Waiting 2 seconds for services to initialize..."
        )
        ...
        log_service_status(
            "STARTUP",
            "ready",
            f"🚀 FastAPI LLM Backend startup completed successfully! ({startup_duration:.2f}s)",
        )
    ...
    async def _initialize_memory_systems(self):
        ...
        try:
            ...
            log_service_status(
                "CACHE_INIT", "starting", "Initializing cache management system"
            )
            ...
            if cache_manager:
                ...
                log_service_status(
                    "CACHE_INIT",
                    "ready",
                    f"Cache management initialized. Version: {cache_stats.get('version', 'unknown')}, Total keys: {cache_stats.get('total_keys', 0)}",
                )
            ...
            log_service_status(
                "MEMORY_HEALTH",
                "starting",
                "Performing startup memory/cache health check...",
            )
            ...
            if retrieved == test_value:
                ...
                startup_duration = time.time() - (
                    self.startup_start_time or time.time()
                )
                log_service_status(
                    "MEMORY_HEALTH",
                    "ready",
                    f"Memory systems healthy (Redis: {redis_status}, ChromaDB: {chroma_status}) - {startup_duration:.2f}s",
                )
                log_service_status(
                    "MEMORY_INIT",
                    "ready",
                    "Memory systems initialized and validated successfully",
                )
            else:
                log_service_status(
                    "MEMORY_HEALTH",
                    "warning",
                    "Cache test failed - systems may be degraded",
                )

            log_service_status(
                "MEMORY", "ready", "Memory and cache systems initialized successfully"
            )

        except Exception as e:
            log_service_status(
                "MEMORY", "error", f"Failed to initialize memory systems: {str(e)}"
            )
        ...
        try:
            ...
            if self.watchdog_thread and self.watchdog_thread.is_alive():
                ...
                log_service_status(
                    "WATCHDOG",
                    "warning",
                    "Background monitoring service failed to start",
                )

        except Exception as e:
            log_service_status(
                "WATCHDOG", "error", f"Failed to start background services: {str(e)}"
            )
    ...
    def _log_system_info(self):
        ...
        try:
            ...
            log_service_status(
                "SYSTEM", "info", f"Platform: {platform.system()} {platform.release()}"
            )
    ...
    async def spinner_log(
        self, message: str, duration: float = 2.0, interval: float = 0.2
    ):
    ...
    def get_config() -> Dict[str, Any]:
        ...
        return {
            ...
            "database": {
                ...
                "use_http_chroma": os.getenv("USE_HTTP_CHROMA", "true").lower()
                == "true",
            },
            "system": {
