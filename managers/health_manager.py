...
from watchdog import get_health_status, get_watchdog
...
async def health_check():
    ...
    log_service_status(
        "HEALTH",
        "checked",
        f"Health check completed: {healthy}/{total} services healthy",
    )
    return response
...
async def detailed_health_check():
    ...
    try:
        ...
        log_service_status(
            "HEALTH",
            "detailed",
            f"Detailed health check: {overall} ({healthy_count} healthy, {degraded_count} degraded, {unhealthy_count} unhealthy)",
        )
...
async def redis_health():
    ...
    try:
        ...
        if redis_monitor:
            ...
            log_service_status(
                "HEALTH", "redis", f"Redis health check: {result.status}"
            )
...
async def chromadb_health():
    ...
    try:
        watchdog = get_watchdog()
        chroma_monitor = next(
            (m for m in watchdog.monitors if m.name == "ChromaDB"), None
        )
        if chroma_monitor:
            result = await chroma_monitor.check_health()
            log_service_status(
                "HEALTH", "chromadb", f"ChromaDB health check: {result.status}"
            )
...
async def service_health_history(service_name: str, hours: int = 24):
    ...
    try:
        ...
        log_service_status(
            "HEALTH",
            "history",
            f"Retrieved {len(history)} health records for {service_name} (last {hours}h)",
        )
        ...
        log_service_status(
            "HEALTH",
            "error",
            f"Failed to get health history for {service_name}: {str(e)}",
        )
        raise HTTPException(
            status_code=500, detail=f"Failed to get health history: {str(e)}"
        )
...
async def storage_health():
    ...
    try:
        ...
        log_service_status(
            "HEALTH",
            "storage",
            f"Storage health: {status} ({existing_dirs}/{total_dirs} dirs, {total_size_mb:.1f}MB)",
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Storage health check failed: {str(e)}"
        )
...
def get_overall_system_health() -> Dict[str, Any]:
    ...
    try:
        ...
        log_service_status(
            "HEALTH", "error", f"Failed to get overall system health: {str(e)}"
        )
        return {
            "status": "error",
            "error": str(e),
