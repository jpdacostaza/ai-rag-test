    ...
    for pyc_file in pyc_files:
        try:
            if os.path.exists(pyc_file):
                os.remove(pyc_file)
                removed_count += 1
                logger.debug("Removed: {pyc_file}")
        except Exception:
            logger.warning("Could not remove {pyc_file}: {e}")

    logger.info("Removed {removed_count} .pyc files")
        ...
        if os.path.exists(file_path):
            try:
                ...
                if "OLLAMA_URL=" in content and "OLLAMA_BASE_URL=" not in content:
                    issues_found.append(
                        "{file_path} ({description}) uses OLLAMA_URL instead of OLLAMA_BASE_URL"
                    )
...
def generate_project_summary():
    ...
    summary = {
        ...
        "config_files": len(glob.glob("*.yml"))
        + len(glob.glob("*.yaml"))
        + len(glob.glob("*.json"))
        + len(glob.glob("*.env*")),
    ...
    logger.info("  - Shell scripts: {summary['shell_scripts']}")
    logger.info("  - Config files: {summary['config_files']}")
    logger.info("  - Documentation files: {summary['documentation']}")
    logger.info("  - Main files size: {summary['total_size_mb']:.2f} MB")
...
def check_requirements():
    ...
    try:
        ...
        if duplicates:
            logger.warning("Duplicate packages found: {duplicates}")
            return False
        else:
            logger.info("✅ requirements.txt looks good ({len(packages)} unique packages)")
            return True

    except Exception:
        logger.error("Error checking requirements.txt: {e}")
        return False
