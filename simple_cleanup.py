        ...
        if os.path.exists(file_path):
            try:
                ...
                if "OLLAMA_URL=" in content and "OLLAMA_BASE_URL=" not in content:
                    issues_found.append(
                        f"{file_path} ({description}) uses OLLAMA_URL instead of OLLAMA_BASE_URL"
                    )
...
def generate_project_summary():
    ...
    main_files = (
        glob.glob("*.py") + glob.glob("*.sh") + glob.glob("*.yml") + glob.glob("*.md")
    )
...
def check_requirements():
    ...
    try:
        ...
        if duplicates:
            ...
            logger.info(
                f"✅ requirements.txt looks good ({len(packages)} unique packages)"
            )
            return True
