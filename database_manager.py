...
from error_handler import RedisConnectionHandler, MemoryErrorHandler
...
class DatabaseManager:
    ...
    def _initialize_redis(self):
        ...
        try:
            ...
            test_client.ping()
            log_service_status("REDIS", "ready", f"Connected to {redis_host}:{redis_port} with Docker-optimized settings")
    ...
    def _initialize_chromadb(self):
        ...
        try:
            ...
            if use_http_chroma:
                ...
                log_service_status("CHROMADB", "ready", f"Using HTTP client connecting to http://{chroma_host}:{chroma_port}")
            else:
                ...
                log_service_status("CHROMADB", "ready", f"Using local file-based client at {chroma_db_dir}")
            ...
            log_service_status("CHROMADB", "ready", f"Successfully connected to ChromaDB and accessed collection '{collection_name}'")
    ...
    def get_redis_client(self):
        ...
        try:
            if self.redis_pool is None:
                log_service_status("REDIS", "reconnecting", "Redis pool not available. Attempting to re-initialize.")
                self._initialize_redis()
                if self.redis_pool is None:
                    return None
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if redis_client:
                    ...
                    log_service_status("REDIS", "failed", f"Could not get Redis client for operation: {operation_name}")
                    if attempt < max_retries:
                        time.sleep(1)  # Wait before retrying
                        continue
                    return None
            except redis.RedisError as e:
                log_service_status("REDIS", "warning", f"Redis operation {operation_name} failed on attempt {attempt + 1}: {e}")
                if attempt >= max_retries:
