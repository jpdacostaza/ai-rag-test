...
class DatabaseManager:
        ...
        try:
            ...
            if use_http_chroma:
                ...
                log_service_status(
                    "CHROMADB",
                    "ready",
                    f"Using local file-based client at {chroma_db_dir}",
                )
            ...
            try:
                ...
                log_service_status(
                    "CHROMADB",
                    "ready",
                    f"Found {len(collections)} existing collections",
                )
            except Exception as e:
                log_service_status(
                    "CHROMADB", "degraded", f"Could not list collections: {e}"
                )
    ...
    def _initialize_embedding_model(self):
        ...
        for model_name in model_candidates:
            try:
                log_service_status(
                    "EMBEDDINGS", "starting", f"Trying model: {model_name}"
                )
                ...
                if test_embedding is not None:
                    log_service_status(
                        "EMBEDDINGS", "ready", f"Successfully loaded {model_name}"
                    )
                    return
            except Exception as e:
                log_service_status(
                    "EMBEDDINGS", "warning", f"Failed to load {model_name}: {e}"
                )
                continue

        log_service_status(
            "EMBEDDINGS", "failed", "All embedding models failed to load"
        )
        self.embedding_model = None
    ...
    def get_redis_client(self):
        ...
        try:
            ...
            log_service_status(
                "REDIS",
                "reconnecting",
                f"Connection issue: {e}. Attempting to re-initialize.",
            )
            self._initialize_redis()
            if self.redis_pool:
                try:
                    ...
                    log_service_status(
                        "REDIS",
                        "failed",
                        "Failed to get a Redis client after re-initialization.",
                    )
            return None
    ...
    def execute_redis_operation(
        self, operation_func, operation_name="redis_op", max_retries=2
    ):
