...
class ComprehensiveTestSuite:
    ...
    def make_request(self, method: str, endpoint: str, headers: Optional[Dict] = None, 
                    data: Any = None, files: Optional[Dict] = None, timeout: int = 30) -> Tuple[Optional[requests.Response], Optional[str]]:
        ...
        try:
            ...
            if method.upper() == "GET":
                ...
                if files:
                    file_headers = {k: v for k, v in req_headers.items() if k != "Content-Type"}
                    response = requests.post(url, headers=file_headers, data=data, files=files, timeout=timeout)
                else:
    ...
    def test_service_availability(self) -> bool:
        ...
        start_time = self.log_test_start("Service Availability", 
        ...
        self.log_test_result("Service Availability", all_ready, 
                           f"Services {'all ready' if all_ready else 'some unavailable'}", duration)
        return all_ready
    
    def test_health_endpoints(self) -> bool:
        """Test all health monitoring endpoints."""
        start_time = self.log_test_start("Health Monitoring", 
        ...
        for endpoint in endpoints:
            ...
            if response and response.status_code == 200:
                ...
                error_msg = error or f"HTTP {response.status_code if response else 'No response'}"
        ...
        duration = time.time() - start_time
        self.log_test_result("Health Monitoring", success, 
                           f"Passed: {passed_count}/{total_count}", duration)
        return success
    
    def test_authentication(self) -> bool:
        """Test authentication middleware."""
        start_time = self.log_test_start("Authentication", 
                                        "Testing API key authentication and access control")
        ...
        invalid_headers = {"X-API-Key": "invalid-key-12345", "Content-Type": "application/json"}
        response, error = self.make_request("GET", "/models", headers=invalid_headers)
        tests["Invalid API Key Rejection"] = response is not None and response.status_code in [401, 403]
        
        # Test 3: No API key
        print("   🔓 Testing without API key...")
        no_auth_headers = {"Content-Type": "application/json"}
        response, error = self.make_request("GET", "/models", headers=no_auth_headers)
        tests["No API Key Rejection"] = response is not None and response.status_code in [401, 403]
        
        # Test 4: Public endpoint access
        print("   🌐 Testing public endpoint access...")
        response, error = self.make_request("GET", "/health/simple", headers=no_auth_headers)
        tests["Public Endpoint Access"] = response is not None and response.status_code == 200
        
        success = all(tests.values())
        details = f"Valid auth: {'✅' if tests.get('Valid API Key') else '❌'}, " + \
    ...
    def test_model_management(self) -> bool:
        ...
        start_time = self.log_test_start("Model Management", 
                                        "Testing model endpoints and availability")
        ...
        tests["OpenAI Compatible"] = response is not None and response.status_code == 200
        ...
        details = f"Models available: {'✅' if tests.get('List Models') else '❌'}, " + \
    ...
    def test_chat_functionality(self) -> bool:
        """Test chat endpoints."""
        start_time = self.log_test_start("Chat Functionality", 
                                        "Testing conversation and chat endpoints")
        ...
        response, error = self.make_request("POST", "/v1/chat/completions", data=openai_data, timeout=45)
        tests["OpenAI Chat"] = response is not None and response.status_code == 200
        
        success = all(tests.values())
        details = f"Basic chat: {'✅' if tests.get('Basic Chat') else '❌'}, " + \
    ...
    def test_document_processing(self) -> bool:
        ...
        start_time = self.log_test_start("Document Processing", 
                                        "Testing file upload and RAG functionality")
        ...
        tests["Supported Formats"] = response is not None and response.status_code == 200
        ...
        try:
            with open(temp_path, 'rb') as file:
                ...
                response, error = self.make_request("POST", "/upload/document", 
                                                  files=files, data=data, timeout=30)
                tests["Document Upload"] = response is not None and response.status_code == 200
        finally:
        ...
        response, error = self.make_request("POST", "/upload/search",
                                          headers={"X-API-Key": self.api_key},
                                          data=search_data, files={})
        tests["Document Search"] = response is not None and response.status_code == 200
        
        success = all(tests.values())
        details = f"Upload: {'✅' if tests.get('Document Upload') else '❌'}, " + \
    ...
    def test_cache_and_storage(self) -> bool:
        ...
        start_time = self.log_test_start("Cache & Storage", 
                                        "Testing cache management and storage operations")
        ...
        # Get cache
        if tests["Cache Set"]:
            response, error = self.make_request("GET", f"/cache/get/{cache_data['key']}")
            if response and response.status_code == 200:
                try:
        ...
        success = all(tests.values())
        details = f"Cache ops: {'✅' if tests.get('Cache Set') and tests.get('Cache Get') else '❌'}, " + \
    ...
    def test_error_handling(self) -> bool:
        ...
        start_time = self.log_test_start("Error Handling", 
                                        "Testing system response to invalid requests")
        ...
        tests["Malformed Request"] = response is not None and response.status_code in [400, 422]
        
        # Test unauthorized access
        print("   🔒 Testing unauthorized access...")
        invalid_headers = {"X-API-Key": "definitely-invalid", "Content-Type": "application/json"}
        response, error = self.make_request("GET", "/models", headers=invalid_headers)
        tests["Auth Rejection"] = response is not None and response.status_code in [401, 403]
        
        success = all(tests.values())
        details = f"404: {'✅' if tests.get('404 Handling') else '❌'}, " + \
    ...
    def test_concurrent_load(self) -> bool:
        ...
        start_time = self.log_test_start("Concurrent Load", 
                                        "Testing system performance under concurrent requests")
        ...
        def make_concurrent_request(request_id: int) -> Dict:
            ...
            req_start = time.time()
            response, error = self.make_request("POST", "/chat", data=chat_data, timeout=20)
        ...
        avg_time = sum(r.get("duration", 0) for r in results if r.get("success", False)) / successful if successful > 0 else 0
    ...
    def generate_final_report(self) -> Dict:
        ...
        print(f"\n{'='*80}")
        print(f"📊 COMPREHENSIVE TEST SUITE - FINAL REPORT")
        print(f"{'='*80}")
