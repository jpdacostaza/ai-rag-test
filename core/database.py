...
from error_handler import MemoryErrorHandler, RedisConnectionHandler, safe_execute
...
def get_cache(db_manager, cache_key, user_id="", request_id=""):
    ...
    return safe_execute(
        lambda: db_manager.execute_redis_operation(
            _get_cache_operation, f"get_cache({cache_key})"
        ),
        fallback_value=None,
...
def set_cache(db_manager, cache_key, value, ttl=600, user_id="", request_id=""):
    ...
    return safe_execute(
        lambda: db_manager.execute_redis_operation(
            _set_cache_operation, f"set_cache({cache_key})"
        ),
...
def get_chat_history(db_manager, user_id, max_history=20, request_id=""):
    ...
    def _get_chat_operation(redis_client):
        ...
        logging.debug(
            f"[MEMORY] Retrieved {len(parsed_history)} chat messages for user {user_id}"
        )
...
def index_document_chunks(db_manager, user_id, doc_id, name, chunks, request_id=""):
    ...
    def _index_op():
        if not db_manager.is_chromadb_available():
            logging.warning(
                "[CHROMADB] ChromaDB not available, skipping document indexing"
            )
            return False
        ...
        try:
            ...
            embeddings = db_manager.embedding_model.encode(
                chunks, show_progress_bar=False
            ).tolist()
            logging.info(
                f"Generated embeddings for {len(chunks)} chunks for doc_id={doc_id}"
            )
...
def retrieve_user_memory(
    db_manager, user_id, query_embedding, n_results=5, request_id=""
):
...
def get_embedding(db_manager, text, request_id=""):
    ...
    return safe_execute(
        _get_embedding,
        fallback_value=None,
        error_handler=lambda e: logging.error(
            f"[EMBEDDINGS] Failed to generate embedding: {e}"
        ),
    )
