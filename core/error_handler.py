...
def get_user_friendly_message(error: Exception, context: str = "") -> str:
    ...
    if isinstance(error, redis.RedisError):
        return (
            "I'm having trouble with the caching service. Your request might be slower."
        )
    ...
    if any(
        keyword in error_str for keyword in ["llm", "ollama", "model", "completion"]
    ):
    ...
    return error_map.get(
        error_type,
        "I encountered an unexpected issue. The details have been logged.",
    )
...
class ToolErrorHandler:
    ...
    def handle_tool_error(
        ...
        return tool_fallbacks.get(
            tool_name, f"The {tool_name} tool encountered an issue."
        )
...
class CacheErrorHandler:
    ...
    def handle_cache_error(
        ...
        log_error(error, context, user_id, request_id)
        logging.warning(
            f"[CACHE] Cache operation failed - continuing without cache: {error}"
        )
