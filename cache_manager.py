        ...
        try:
            ...
            if stored_version != self.CACHE_VERSION:
                ...
                log_service_status(
                    "CACHE",
                    "ready",
                    f"Cache upgraded to version {self.CACHE_VERSION}",
                )
        ...
        try:
            ...
            if chat_keys:
                ...
                log_service_status(
                    "CACHE",
                    "ready",
                    f"Invalidated {deleted} chat cache entries",
                )
            else:
                log_service_status(
                    "CACHE", "ready", "No chat cache entries to invalidate"
                )
        except Exception as e:
            log_service_status(
                "CACHE", "error", f"Failed to invalidate chat cache: {e}"
            )
        ...
        try:
            ...
            while True:
                ...
                filtered_keys = [
                    k
                    for k in keys
                    if k not in [self.VERSION_KEY, self.SYSTEM_PROMPT_HASH_KEY]
                ]
            ...
            if all_keys:
                ...
                log_service_status(
                    "CACHE",
                    "ready",
                    f"Invalidated all cache: {deleted} entries deleted",
                )
            else:
    ...
    def validate_response_format(self, response: str) -> bool:
        ...
        if response_trimmed.startswith("{") and response_trimmed.endswith("}"):
            try:
                ...
                # If it parses as JSON, it's invalid for our plain text requirement
                log_service_status(
                    "CACHE",
                    "warning",
                    "Detected JSON response format - invalidating",
                )
                return False
        ...
        try:
            if not self.validate_response_format(value):
                log_service_status(
                    "CACHE",
                    "warning",
                    f"Skipping cache for key {key} - invalid format",
                )
                return False
            ...
            log_service_status(
                "CACHE",
                "ready",
                f"CACHE SET - Key: {key}, TTL: {ttl}s, Value: {value_preview}",
            )

            return True
        except Exception as e:
            log_service_status(
                "CACHE", "error", f"Failed to set cache for key {key}: {e}"
            )
            return False
    ...
    def get_with_validation(self, key: str) -> Optional[str]:
        ...
        try:
            ...
            try:
                ...
                if isinstance(data, dict) and "value" in data:
                    ...
                    if not self.validate_response_format(data["value"]):
                        log_service_status(
                            "CACHE",
                            "warning",
                            f"Invalidating cache key {key} - bad format",
                        )
                    ...
                    log_service_status(
                        "CACHE",
                        "ready",
                        f"CACHE HIT - Key: {key}, Cached: {cached_at}, Value: {value_preview}",
                    )
                    ...
                    if not self.validate_response_format(cached_data):
                        log_service_status(
                            "CACHE",
                            "warning",
                            f"Invalidating old format cache key {key}",
                        )
                        self.redis_client.delete(key)
                        return None
                    ...
                    log_service_status(
                        "CACHE",
                        "ready",
                        f"CACHE HIT (legacy) - Key: {key}, Value: {value_preview}",
                    )
                ...
                value_preview = (
                    cached_data[:50] + "..." if len(cached_data) > 50 else cached_data
                )
                log_service_status(
                    "CACHE",
                    "ready",
                    f"CACHE HIT (string) - Key: {key}, Value: {value_preview}",
                )
            ...
            log_service_status(
                "CACHE", "error", f"Failed to get cache for key {key}: {e}"
            )
            return None
