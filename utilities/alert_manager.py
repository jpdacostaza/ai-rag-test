...
class AlertManager:
    ...
    def _setup_default_thresholds(self):
        ...
        self.alert_thresholds = {
            ...
            "response_time": {"medium": 3000, "high": 5000, "critical": 10000},  # 3 seconds  # 5 seconds  # 10 seconds
            "error_rate": {
                "medium": 1.0,  # 1% error rate
                "high": 5.0,  # 5% error rate
                "critical": 10.0,  # 10% error rate
            },
            "service_downtime": {"medium": 60, "high": 300, "critical": 900},  # 1 minute  # 5 minutes  # 15 minutes
        }
    ...
    async def resolve_alert(self, alert_id: str, resolution_message: str = "") -> bool:
        ...
        log_service_status("alert_manager", "info", f"Alert resolved: {alert.title}")
    ...
    async def _send_notifications(self, alert: Alert, channels: List[AlertChannel]):
        ...
        for channel in channels:
            try:
                ...
                log_service_status(
                    "alert_manager", "error", f"Failed to send notification via {channel.value}: {str(e)}"
                )
    ...
    async def _send_console_notification(self, alert: Alert):
        ...
        if alert.metrics:
            print(f"{color}[CHART] Metrics: {json.dumps(alert.metrics, indent=2)}{reset}")
...
"""
            ...
            for email in config["to_emails"]:
                if email.strip():
                    ...
                    server.sendmail(config["from_email"] or config["smtp_user"], email.strip(), msg)
    ...
    async def _send_slack_notification(self, alert: Alert):
        ...
        if alert.metrics:
            payload["attachments"][0]["fields"].append(
                {"title": "Metrics", "value": f"```{json.dumps(alert.metrics, indent=2)}```", "short": False}
            )
        ...
        try:
            ...
            async with http_client() as client:
                response = await client.post(config["webhook_url"], json=payload, timeout=10)
    ...
    async def _send_webhook_notification(self, alert: Alert):
        ...
        payload = {"alert": alert.to_dict(), "timestamp": alert.timestamp.isoformat(), "source": "llm_backend_monitor"}

        try:
            from utilities.async_context_managers import http_client
            async with http_client() as client:
                response = await client.post(config["url"], json=payload, headers=config.get("headers", {}), timeout=10)
                response.raise_for_status()
    ...
    def get_alert_stats(self) -> Dict:
        ...
        for severity in AlertSeverity:
            severity_counts[severity.value] = len([alert for alert in recent_alerts if alert.severity == severity])
    ...
    def _calculate_avg_resolution_time(self) -> Optional[float]:
        ...
        resolved_alerts = [
            alert for alert in self.alert_history if alert.resolved and alert.resolution_time is not None
        ]
    ...
    async def cleanup_old_alerts(self, days: int = 7):
        ...
        # Remove from history
        self.alert_history = [alert for alert in self.alert_history if alert.timestamp >= cutoff or not alert.resolved]

        # Remove from active alerts if resolved and old
        old_resolved = [
            alert_id for alert_id, alert in self.alerts.items() if alert.resolved and alert.timestamp < cutoff
        ]

        for alert_id in old_resolved:
            del self.alerts[alert_id]

        log_service_status("alert_manager", "info", f"Cleaned up {len(old_resolved)} old resolved alerts")
...
async def alert_memory_pressure(percentage: float, component: str = "system"):
    ...
    if percentage >= 95:
        ...
        actions = ["Immediate memory cleanup required", "Consider scaling up resources", "Check for memory leaks"]
    elif percentage >= 85:
        severity = AlertSeverity.HIGH
        actions = ["Monitor memory usage closely", "Prepare for potential scaling", "Review cache settings"]
    else:
        severity = AlertSeverity.MEDIUM
        actions = ["Monitor memory trends", "Review memory usage patterns"]
...
async def alert_cache_performance(hit_rate: float, component: str = "cache"):
    ...
    if hit_rate < 10:
        severity = AlertSeverity.CRITICAL
        actions = ["Investigate cache configuration", "Check cache eviction policies", "Review cache key strategies"]
    elif hit_rate < 30:
        severity = AlertSeverity.HIGH
        actions = ["Review cache settings", "Analyze cache usage patterns", "Consider cache warming strategies"]
    else:
        severity = AlertSeverity.MEDIUM
        actions = ["Monitor cache performance trends", "Review cache optimization opportunities"]
