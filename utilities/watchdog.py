...
class SubsystemMonitor:
    ...
    def should_alert(self) -> bool:
        ...
        return self.consecutive_failures >= self.config.alert_threshold and not self.alert_sent
...
class ChromaDBMonitor(SubsystemMonitor):
    ...
    async def check_health(self) -> ServiceHealth:
        ...
        try:
            # Use ConnectionFactory for ChromaDB client
            ...
            if results and len(results.get("ids", [[]])[0]) > 0:
                ...
                # Get ChromaDB config from environment (same as ConnectionFactory uses)
                use_http = os.getenv("USE_HTTP_CHROMA", "false").lower() == "true"
                metadata = {
                ...
                if use_http:
                    ...
                    metadata.update({"mode": "file", "persist_directory": os.getenv("CHROMA_DB_DIR", "./storage/chroma")})
