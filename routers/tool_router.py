...
class ToolRouter:
    ...
    def _is_time_query(self, user_message: str) -> bool:
        ...
        return (
            ...
            or any(re.search(pat, user_message, re.IGNORECASE) for pat in time_patterns)
        )
    ...
    def _is_python_code_query(self, user_message: str) -> bool:
        ...
        return (
            ...
            or "```python" in user_message
        )

    def _execute_time_tool(
        self, user_message: str, user_id: str, request_id: str, debug_info: List[str]
    ...
    def _execute_exchange_rate_tool(
        ...
        if match:
            ...
            debug_info.append(
                f"[TOOL] Exchange rate lookup for {from_cur} to {to_cur} (disabled)"
            )
        else:
            user_response = (
                "Please specify exchange rate like 'exchange rate USD to EUR'."
            )
    ...
    def _execute_python_code_tool(
        ...
        if not code:
            ...
            debug_info.append(
                f"[TOOL] Python code execution disabled (code: {code[:50]}...)"
            )
