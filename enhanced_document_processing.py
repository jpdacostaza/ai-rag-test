...
class DocumentAnalyzer:
    ...
    async def analyze_document(
        ...
        try:
            ...
            log_service_status(
                "DOC_ANALYSIS", "error", f"Document analysis failed: {e}"
            )
    ...
    def _classify_document_type(self, content: str, file_ext: str) -> DocumentType:
        ...
        if (
            sum(1 for indicator in academic_indicators if indicator in content_lower)
            >= 3
        ):
    ...
    def _calculate_complexity(self, content: str) -> float:
        ...
        tech_score = sum(
            1 for term in technical_indicators if term in content.lower()
...
class EnhancedChunker:
    ...
    async def _create_processed_chunk(
        ...
        chunk_id = (
            f"{user_id}_{hashlib.md5((filename + str(index)).encode()).hexdigest()[:8]}"
        )
    ...
    def _assess_chunk_quality(self, text: str, strategy: ChunkingStrategy) -> float:
        ...
        completeness = (
            1.0 if text.strip().endswith((".", "!", "?", "```", "}", ")", "]")) else 0.7
        )
        ...
        meaningful_words = [
            ...
            for w in words
            if len(w) > 3
            and not w.lower() in {"the", "and", "that", "this", "with", "from"}
        ]
    ...
    async def _fallback_processing(
        ...
        for i, chunk_text in enumerate(chunks):
            chunk_id = (
                f"{user_id}_{hashlib.md5((filename + str(i)).encode()).hexdigest()[:8]}"
            )
