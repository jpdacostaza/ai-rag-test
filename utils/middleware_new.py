...
class AuthenticationMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, skip_auth_paths: Optional[Set[str]] = None):
        ...
        log_service_status(
            "MIDDLEWARE",
            "ready",
            f"Authentication middleware initialized with {len(self.skip_auth_paths)} skip paths",
        )
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        log_service_status(
            "AUTH", "debug", f"🔐 Processing {request.method} {request.url.path}"
        )

        # Check if this path should skip authentication
        should_skip = self._should_skip_auth(request)
        print(f"DEBUG: should_skip_auth={should_skip} for path {request.url.path}")

        if should_skip:
            print("DEBUG: Skipping authentication for this path")
        ...
        print(
            f"DEBUG: Extracted API key: {api_key[:10] + '...' if api_key else 'None'}"
        )
        ...
        if not is_valid:
            ...
            error_response = JSONResponse(
                status_code=401, content=auth_manager.create_auth_error_response()
            )
            self._log_request(request, error_response, start_time, auth_failed=True)
            return error_response

        print("DEBUG: Authentication successful - proceeding with request")
        # Authentication successful, proceed with request
        response = await call_next(request)
        self._log_request(
            request, response, start_time, auth_success=True, api_key=api_key
        )
        return response
    ...
    def _extract_api_key_from_request(self, request: Request) -> Optional[str]:
        ...
        """
        # Try X-API-Key header first (most common for this API)
    ...
    def _log_request(
        ...
        log_details = {
            ...
            "auth_status": (
                ...
                if auth_skipped
                    ...
                    "failed"
                    if auth_failed
                    else ("success" if auth_success else "unknown")
                )
            ),
        }
        ...
        if auth_failed:
            log_service_status(
                "AUTH",
                "warning",
                f"❌ {request.method} {request.url.path} → {response.status_code} (auth failed)",
            )
        elif auth_success:
            log_service_status(
                "AUTH",
                "success",
                f"✅ {request.method} {request.url.path} → {response.status_code} (authenticated)",
            )
        else:
            log_api_request(
...
class RequestLoggingMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app):
        super().__init__(app)
        log_service_status(
            "MIDDLEWARE", "ready", "Request logging middleware initialized"
        )
