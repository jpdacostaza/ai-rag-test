...
class AuthenticationManager:
    ...
    def validate_api_key(self, api_key: Optional[str]) -> bool:
        ...
        if is_valid:
            log_service_status(
                "AUTH", "success", f"Valid API key used: {api_key[:10]}..."
            )
        else:
            log_service_status(
                "AUTH",
                "warning",
                f"Invalid API key attempted: {api_key[:10] if api_key else 'None'}...",
            )
    ...
    def _track_failed_attempt(self, api_key: Optional[str]):
        ...
        self.failed_attempts[key] = [
            attempt for attempt in self.failed_attempts[key] if attempt > cutoff
        ]

        # Log if too many failed attempts
        if len(self.failed_attempts[key]) > 5:
            log_service_status(
                "AUTH", "error", f"Multiple failed auth attempts from key: {key}"
            )

    def extract_api_key(
        self, authorization: Optional[str] = None, api_key_param: Optional[str] = None
    ) -> Optional[str]:
        """
...
def extract_api_key_from_request(
    authorization: Optional[str] = None, api_key_param: Optional[str] = None
) -> Optional[str]:
    """
