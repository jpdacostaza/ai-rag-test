# Docker Restart and Monitor Script
param(
...
while ((Get-Date) -lt $endTime) {
    ...
    Write-Log "Checking for errors in recent logs..." "INFO"
    foreach ($container in $containers) {
        try {
            $errorLogs = docker logs --since="30s" $container 2>&1 | Where-Object { $_ -match "(error|failed|exception|traceback)" -and $_ -notmatch "(?i)(test|debug)" }
            if ($errorLogs) {
                Write-Log "ERRORS detected in ${container}:" "ERROR"
                $errorLogs | ForEach-Object { Write-Log "   ERROR: $_" "ERROR" }
            }
        } catch {
...
# Final status report
Write-Log "========================================" "INFO"
Write-Log "Final container status report:" "INFO"
Write-Log "========================================" "INFO"
docker ps -a 2>&1 | ForEach-Object { Write-Log $_ "REPORT" }

Write-Log "========================================" "INFO"
Write-Log "Docker compose services status:" "INFO"
docker-compose ps 2>&1 | ForEach-Object { Write-Log $_ "REPORT" }

Write-Log "Monitoring completed successfully!" "INFO"
