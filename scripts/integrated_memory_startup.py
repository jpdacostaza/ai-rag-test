...
# Import error handling patterns for integrated startup consistency
try:
...
class FunctionManager:
    ...
    def install_function(self, function_code: str) -> bool:
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO function (id, user_id, name, type, content, is_active, is_global, meta, valves, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                "memory_function",
                "",  # Global function
    ...
    def install_pipeline_connection(self) -> bool:
        """Install pipeline connection in OpenWebUI database."""
        try:
            db_path = Path(OPENWEBUI_DB_PATH)
            if not db_path.exists():
                self.logger.log("Database not found for pipeline connection", "ERROR")
                return False
            
            conn = sqlite3.connect(str(db_path))
            cursor = conn.cursor()
            
            # First, let's examine the actual database schema
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table'")
            tables = [row[0] for row in cursor.fetchall()]
            self.logger.log(f"Available tables: {tables}", "DEBUG")
            
            # Check the actual schema of config table
            cursor.execute("PRAGMA table_info(config)")
            columns = cursor.fetchall()
            self.logger.log(f"Config table schema: {columns}", "DEBUG")
            ...
            # Check if pipeline connection already exists - query by data content since id is auto-increment
            cursor.execute("SELECT id FROM config WHERE json_extract(data, '$.type') = ?", ("pipelines",))
            existing = cursor.fetchone()
            
            # Create pipeline connection configuration as proper JSON
            pipeline_config = {
                "type": "pipelines",
                ...
                "description": "Memory enhancement pipeline connection"
            }
            
            if existing:
                # Update existing config using the id
                cursor.execute("""
                    UPDATE config SET data = ?, version = version + 1, updated_at = CURRENT_TIMESTAMP 
                    WHERE id = ?
                """, (json.dumps(pipeline_config), existing[0]))
                self.logger.log("Pipeline connection updated", "SUCCESS")
            else:
                # Insert new config - let id auto-increment, provide required fields
                cursor.execute("""
                    INSERT INTO config (data, version, created_at, updated_at)
                    VALUES (?, 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
                """, (json.dumps(pipeline_config),))
                self.logger.log("Pipeline connection installed", "SUCCESS")
            ...
            self.logger.log(f"Pipeline connection installation failed: {e}", "ERROR")
            # Log more details about the error
            import traceback
            self.logger.log(f"Full error traceback: {traceback.format_exc()}", "DEBUG")
            return False
...
class PipelineManager:
    ...
    @handle_api_errors()
    def wait_for_pipelines(self) -> bool:
        """Wait for pipelines service to be ready."""
        self.logger.log("Waiting for Pipelines service...", "INFO")
        ...
        while time.time() - start_time < timeout:
            try:
                response = requests.get("http://pipelines:9099/health", timeout=10)
                if response.status_code == 200:
                    self.logger.log("Pipelines service is ready!", "SUCCESS")
                    return True
                elif response.status_code == 404:
                    # Try root endpoint if health endpoint doesn't exist
                    response = requests.get("http://pipelines:9099/", timeout=10)
                    if response.status_code == 200:
                        self.logger.log("Pipelines service is ready!", "SUCCESS")
                        return True
                self.logger.log(f"Pipelines responded with status {response.status_code}", "DEBUG")
            except Exception as e:
                self.logger.log(f"Pipelines not ready: {e}", "DEBUG")
            
            time.sleep(10)
        
        self.logger.log(f"Pipelines failed to become ready within {timeout}s", "WARNING")
        return False
...
class IntegratedMemoryAPI:
    ...
    def start_memory_api(self):
        ...
        try:
            ...
            # Create simple FastAPI app for memory API
            app = FastAPI()
            memory_service = create_memory_service(MemoryProviderType.API)
            ...
            async def retrieve_memories(user_id: str, query: str = None):
                return await memory_service.get_relevant_memories(user_id, query)
