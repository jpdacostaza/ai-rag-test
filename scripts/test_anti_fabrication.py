        ...
        if os.path.exists(file_path):
            ...
            try:
                ...
                if fabrication_count > 0:
                    print(f"  ❌ FABRICATION RISK: Found {fabrication_count} risky patterns")
                    print(f"  🛡️ Safety measures: {safety_count}")
                    
                    if safety_count == 0:
                        print(f"  🚨 HIGH RISK: No anti-fabrication measures found!")
                    else:
                        print(f"  ⚠️ MODERATE RISK: Some safety measures present")
                else:
                    print(f"  ✅ SAFE: No fabrication indicators found")
                    print(f"  🛡️ Safety measures: {safety_count}")
...
def test_memory_processor():
    ...
    try:
        ...
        if "never fabricate" in small_persona.lower():
            print("✅ Small model persona has anti-fabrication measures")
        else:
            print("❌ Small model persona lacks anti-fabrication measures")
        ...
        if "only when provided" in new_user_persona.lower():
            print("✅ New user persona has proper memory handling")
        else:
            print("❌ New user persona may cause fabrication")
            
    except Exception as e:
        print(f"❌ Error testing memory processor: {e}")
