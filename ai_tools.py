...
def get_weather_weatherapi(city: str = "London") -> str:
    ...
    try:
        ...
        if resp.status_code != 200 or "error" in data:
            ...
            return f"WeatherAPI.com error: {error_msg}"
        c = data["current"]
        loc = data["location"]
        return (
            f"Weather in {loc['name']}, {loc['country']}: {c['temp_c']}°C, "
            f"{c['condition']['text']}, wind {c['wind_kph']} kph, "
...
def get_weather(city: str = "London") -> str:
    ...
    try:
        ...
        return (
            f"Weather in {city}: {w.get('temperature', '?')}°C, "
            f"wind {w.get('windspeed', '?')} km/h, {w.get('weathercode', '?')}"
        )
    except Exception as e:
...
def chunk_text(
    ...
    try:
        ...
        logging.debug(
            f"[CHUNKING] Created {len(chunks)} chunks from text of length {len(text)}"
        )
        return chunks
    except Exception as e:
...
def convert_units(value: float, from_unit: str, to_unit: str) -> str:
    ...
    try:
        ...
        if from_unit in ["celsius", "c"] and to_unit in ["fahrenheit", ""]:
            result = (value * 9 / 5) + 32
        ...
        logging.error(
            f"[CONVERSION] Error converting {value} {from_unit} to {to_unit}: {e}"
        )
...
def wikipedia_search(query: str, sentences: int = 3) -> str:
    ...
    try:
        ...
        try:
            ...
            logging.error(
                f"[WIKIPEDIA] Fallback disambiguation failed for {query}: {fallback_e}"
            )
...
def web_search(query: str, num_results: int = 5) -> str:
    ...
    try:
        ...
        if data.get("AbstractText"):
            ...
            return (
                f"No detailed results found for '{query}'. Try a more specific search."
            )
