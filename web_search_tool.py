...
class WebSearchTool:
    ...
    async def _get_session(self) -> aiohttp.ClientSession:
        ...
        if self.session is None or self.session.closed:
            ...
            connector = aiohttp.TCPConnector(
                limit=10, ttl_dns_cache=300, use_dns_cache=True  # Connection pool limit  # DNS cache for 5 minutes
            )
    ...
    async def search(self, query: str, max_results: int = 3) -> Dict[str, Any]:
        ...
        try:
            ...
            async with asyncio.timeout(WEB_SEARCH_TIMEOUT):
                ...
                if results:
                    logger.info(
                        f"Web search successful: '{query}' returned {len(results)} results in {search_duration:.3f}s"
                    )
                else:
    ...
    def should_search_web(self, user_message: str, model_response: str = "") -> bool:
        ...
        is_casual_greeting = (
            any(greeting in user_lower for greeting in casual_greetings) and len(user_message.split()) < 10
        )

        is_tool_query = any(keyword in user_lower for keyword in tool_specific_keywords)

        is_news_query = any(keyword in user_lower for keyword in news_keywords)
        ...
        if should_search:
            logger.info(
                f"Web search triggered - Current: {needs_current_info}, Factual: {needs_factual_info}, Uncertain: {model_uncertain}"
            )
