...
def quick_chromadb_check() -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            try:
                test_collection = db_manager.chroma_client.get_or_create_collection(
...
def startup_memory_health_check() -> Dict[str, Any]:
    ...
    """
    log_service_status(
        "MEMORY_HEALTH", "starting", "Performing startup memory/cache health check..."
    )
    ...
    if overall_status == "healthy":
        log_service_status(
            "MEMORY_HEALTH",
            "ready",
            f"Memory systems healthy (Redis: ✅, ChromaDB: ✅) - {duration:.2f}s",
        )
        ...
        log_service_status(
            "MEMORY_HEALTH",
            "degraded",
            f"Memory systems degraded (Redis: {redis_status}, ChromaDB: {chromadb_status}) - {duration:.2f}s",
        )
    else:
        log_service_status(
            "MEMORY_HEALTH",
            "failed",
            f"Memory systems failed (Redis: ❌, ChromaDB: ❌) - {duration:.2f}s",
        )
...
def initialize_memory_systems() -> bool:
    ...
    try:
        ...
        overall_success = cache_init_success and health_result["overall_status"] in [
            "healthy",
            "degraded",
        ]
        ...
        if overall_success:
            ...
            log_service_status(
                "MEMORY_INIT",
                "warning",
                "Memory systems initialization completed with issues",
            )
        ...
        log_service_status(
            "MEMORY_INIT", "failed", f"Memory systems initialization failed: {e}"
        )
        return False
...
if __name__ == "__main__":
    ...
    print(
        f"ChromaDB: {result['chromadb']['status']} - {result['chromadb'].get('error', 'OK')}"
    )
