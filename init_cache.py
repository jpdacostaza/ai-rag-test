...
def initialize_cache_management():
    ...
    try:
        ...
        if not cache_manager:
            log_service_status(
                "CACHE_INIT", "failed", "Could not initialize cache manager"
            )
            return False
        ...
        log_service_status(
            "CACHE_INIT",
            "ready",
            f"Cache management initialized. Version: {stats.get('version', 'unknown')}, Total keys: {stats.get('total_keys', 0)}",
        )
