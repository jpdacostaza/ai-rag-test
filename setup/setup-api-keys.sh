...
check_updated_tools() {
    ...
    local tools=(
        ...
        "../demo-tests/debug-tools/test_memory_cross_chat.py"
    )
    
    for tool in "${tools[@]}"; do
        if [[ -f "$SCRIPT_DIR/$tool" ]]; then
            # Check if tool imports API key manager
            if grep -q "from setup.api_key_manager import" "$SCRIPT_DIR/$tool" 2>/dev/null; then
...
show_usage_examples() {
    ...
    echo -e "      ${CYAN}python3 ../demo-tests/debug-tools/openwebui_memory_diagnostic.py${NC}"
    
    echo -e "\n   ${INFO} Run with specific user:"
    echo -e "      ${CYAN}python3 ../demo-tests/debug-tools/openwebui_memory_diagnostic.py --user=john${NC}"
    
    echo -e "\n   ${INFO} Run with specific environment:"
    echo -e "      ${CYAN}python3 ../demo-tests/debug-tools/openwebui_memory_diagnostic.py --env=production${NC}"
    
    echo -e "\n   ${INFO} Test memory across chat sessions:"
    echo -e "      ${CYAN}python3 ../demo-tests/debug-tools/test_memory_cross_chat.py${NC}"
}
...
test_setup() {
    ...
    if [[ -f "$SCRIPT_DIR/../demo-tests/debug-tools/openwebui_memory_diagnostic.py" ]]; then
        echo -e "${INFO} Running diagnostic tool..."
        python3 "$SCRIPT_DIR/../demo-tests/debug-tools/openwebui_memory_diagnostic.py" || \
