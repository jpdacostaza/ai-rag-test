...
class BackendCleanup:
    ...
    def log_action(self, action: str, file_path: str = "", size: int = 0):
        """Log cleanup actions."""
        if self.dry_run:
            logger.info("[DRY RUN] {action}: {file_path}")
        else:
            logger.info("{action}: {file_path}")
            if size > 0:
        ...
        for pattern in temp_patterns:
            ...
            for temp_file in temp_files:
                if os.path.exists(temp_file):
                    ...
                    self.log_action("Removing temp file", temp_file, size)
                    if not self.dry_run:
                        os.remove(temp_file)
                    self.cleanup_stats["files_removed"] += 1
    ...
    def validate_python_files(self):
        ...
        for py_file in python_files:
            try:
                ...
                logger.debug("✅ {py_file}")
            except SyntaxError:
                logger.error("❌ Syntax error in {py_file}: {e}")
                invalid_files.append(py_file)
            except Exception:
                logger.warning("⚠️ Could not validate {py_file}: {e}")

        if invalid_files:
            logger.error("Found {len(invalid_files)} files with syntax errors!")
            return False
        else:
            logger.info("✅ All {len(python_files)} Python files are valid")
            return True
        ...
        for py_file in python_files:
            try:
                ...
                if module_name not in ["setup", "comprehensive_cleanup"]:  # Skip setup files
                    logger.debug("Testing imports for {py_file}")
                    # Note: In production, you'd use ast parsing for safer
                    # checking
            except Exception:
                logger.debug("Could not test imports for {py_file}: {e}")
    ...
    def optimize_requirements(self):
        ...
        if os.path.exists(req_file):
            ...
            if duplicates:
                logger.warning("Found duplicate packages in requirements.txt: {duplicates}")
            else:
    ...
    def check_configuration_consistency(self):
        ...
        config_files = ["docker-compose.yml", ".env.example", "README.md"]
            ...
            if os.path.exists(config_file):
                try:
                    with open(config_file, "r", encoding="utf-8") as f:
                        ...
                        if "OLLAMA_URL=" in content and "OLLAMA_BASE_URL=" not in content:
                            inconsistencies.append(
                                "{config_file} uses OLLAMA_URL instead of OLLAMA_BASE_URL"
                            )
                    ...
                    try:
                        ...
                        logger.warning("Could not read {config_file}: {e}")
                        continue
    ...
    def generate_file_inventory(self):
        ...
        inventory = {
            ...
            "scripts": glob.glob("**/*.sh", recursive=True) + glob.glob("**/*.ps1", recursive=True),
            "total_files": 0,
            "total_size": 0,
        }
        ...
        logger.info("  - Python files: {len(inventory['python_files'])}")
        logger.info("  - Config files: {len(inventory['config_files'])}")
        logger.info("  - Documentation: {len(inventory['documentation'])}")
        logger.info("  - Scripts: {len(inventory['scripts'])}")
        logger.info("  - Total files: {inventory['total_files']}")
        logger.info("  - Total size: {total_size / (1024*1024):.2f} MB")
    ...
    def run_cleanup(self):
        ...
        logger.info("  - Files removed: {self.cleanup_stats['files_removed']}")
        logger.info("  - Directories cleaned: {self.cleanup_stats['directories_cleaned']}")
        logger.info("  - Space saved: {self.cleanup_stats['bytes_saved'] / 1024:.2f} KB")
