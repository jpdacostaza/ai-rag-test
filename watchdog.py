...
class WatchdogConfig:
    ...
    def __post_init__(self):
        ...
        try:
            self.enable_logging = (
                os.getenv("WATCHDOG_ENABLE_LOGGING", "true").lower() == "true"
            )
        except (ValueError, TypeError):
        ...
        try:
            ...
            stable_interval_str = (
                stable_interval_str.split()[0] if stable_interval_str else "60"
            )
    ...
    async def check_health(self) -> ServiceHealth:
        ...
        try:
            ...
            if self.use_http_chroma:
                ...
                client = chromadb.Client(Settings(persist_directory=self.chroma_dir))
            ...
            if results and len(results.get("ids", [[]])[0]) > 0:
                ...
                if self.use_http_chroma:
                    ...
                    metadata.update(
                        {"mode": "file", "persist_directory": self.chroma_dir}
                    )
...
class EmbeddingMonitor(SubsystemMonitor):
    ...
    async def check_health(self) -> ServiceHealth:
        ...
        try:
            ...
            if (
                not db_manager.is_embeddings_available()
                or db_manager.embedding_model is None
            ):
            ...
            test_embedding = await asyncio.to_thread(
                embedding_model.encode, [test_text]
            )
...
class SystemWatchdog:
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "services": {
                name: asdict(health) for name, health in latest_results.items()
            },
    ...
    def _store_watchdog_metrics(
        self, check_duration: float, healthy_count: int, total_count: int
    ):
        ...
        self.watchdog_metrics = [
            m for m in self.watchdog_metrics if m["timestamp"] > cutoff_time
        ]
...
if __name__ == "__main__":
    ...
    async def main():
        config = WatchdogConfig(
            check_interval=10, enable_logging=True, log_level="DEBUG"
        )
