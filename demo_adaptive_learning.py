...
async def demo_adaptive_learning():
    ...
    if insights.get("status") == "success":
        ...
        print(
            f"Average Context Relevance: {user_insights.get('avg_context_relevance', 0):.2f}"
        )
        print()
        ...
        for feedback_type, count in user_insights.get(
    ...
    if user_patterns:
        ...
        if topics:
            print(
                f"  Favorite Topics: {dict(sorted(topics.items(), key=lambda x: x[1], reverse=True)[:3])}"
            )

        tools = user_patterns.get("tool_preferences", {})
        if tools:
            print(
                f"  Tool Preferences: {dict(sorted(tools.items(), key=lambda x: x[1], reverse=True)[:3])}"
            )

        preferences = user_patterns.get("preferences", {})
        if preferences:
            print(
                f"  Detail Level Preference: {preferences.get('detail_level', 0.5):.2f}"
            )
            if preferences.get("preferred_response_time"):
                avg_time = sum(preferences["preferred_response_time"]) / len(
