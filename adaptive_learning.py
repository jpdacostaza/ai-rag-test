...
class ConversationAnalyzer:
    ...
    async def _calculate_context_relevance(
        ...
        try:
            ...
            query_memory_overlap = len(query_words & memory_words) / max(
                len(query_words), 1
            )
            ...
            log_service_status(
                "LEARNING", "error", f"Context relevance calculation failed: {e}"
            )
            return 0.5
    ...
    def _count_follow_up_indicators(self, message: str) -> int:
        ...
        message_lower = message.lower()
        return sum(
            1 for indicator in follow_up_indicators if indicator in message_lower
        )
...
class AdaptiveLearningSystem:
    ...
    def __init__(self):
        ...
        self.learning_metrics: Dict[str, deque] = defaultdict(
    ...
    async def process_interaction(
        ...
        try:
            ...
            log_service_status(
                "LEARNING",
                "ready",
                f"Processed interaction for {user_id}: {feedback_type_str}",
            )

            return {
                "status": "success",
                "feedback_type": (
                    metrics.feedback_type.value if metrics.feedback_type else None
                ),
    ...
    async def get_user_insights(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            if user_id not in self.user_patterns:
                return {
                    "status": "no_data",
                    "message": "No learning data available for user",
                }

            user_data = self.user_patterns[user_id]
            recent_metrics = list(self.learning_metrics[user_id])[
                -10:
            ...
            if recent_metrics:
                avg_context_score = sum(
                    m.context_relevance_score for m in recent_metrics
                ) / len(recent_metrics)
            ...
            return {
                ...
                "insights": {
                    ...
                    "learning_trend": (
                        "improving" if avg_context_score > 0.6 else "stable"
                    ),
                },
            }
    ...
    async def _update_learning_patterns(
        self, user_id: str, metrics: InteractionMetrics
    ):
    ...
    async def _check_knowledge_expansion(
        ...
        try:
            ...
            if metrics.feedback_type == FeedbackType.CORRECTION:
                should_expand = True
                expansion_content = (
                    f"User correction: {user_message}\nContext: {assistant_response}"
                )
                ...
                should_expand = True
                expansion_content = (
                    f"High-quality interaction: Q: {user_message}\n"
                    f"A: {assistant_response}"
                )
                ...
                should_expand = True
                expansion_content = f"User-requested learning: {user_message}"
            ...
            log_service_status(
                "LEARNING", "error", f"Knowledge expansion check failed: {e}"
            )

    async def _process_knowledge_expansion(
        self, user_id: str, content: str, expansion_type: str
    ):
        ...
        try:
            ...
            if success:
                ...
                log_service_status(
                    "LEARNING",
                    "error",
                    f"Failed to expand knowledge base for {user_id}",
                )

        except Exception as e:
            log_service_status(
                "LEARNING", "error", f"Knowledge expansion processing failed: {e}"
            )
    ...
    async def _update_user_preferences(self, user_id: str, metrics: InteractionMetrics):
        ...
        try:
            ...
            if metrics.feedback_type == FeedbackType.POSITIVE:
                ...
                preferences["preferred_response_time"] = preferences[
                    "preferred_response_time"
                ][
                    -10:
                ]  # Keep last 10
            ...
            if metrics.follow_up_questions > 2:
                ...
                metrics.follow_up_questions == 0
                and metrics.feedback_type == FeedbackType.POSITIVE
            ):
                preferences["detail_level"] = (
                    preferences.get("detail_level", 0.5) - 0.05
                )

            # Normalize detail level between 0 and 1
            preferences["detail_level"] = max(
                0, min(1, preferences.get("detail_level", 0.5))
            )
