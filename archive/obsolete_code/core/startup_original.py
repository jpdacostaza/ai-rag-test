...
import httpx
from config.config_unified import DEFAULT_MODEL, OLLAMA_BASE_URL, log_system_info, log_environment_variables
...
from services.model_manager import ensure_model_available
from routes.models import refresh_model_cache
from utilities.watchdog import start_watchdog_service
...
async def startup_event(app) -> None:
    ...
    try:
        ...
        if db_manager and hasattr(db_manager, "redis_client") and db_manager.redis_client is not None:
            ...
            log_service_status("STARTUP", "degraded", "Redis client not available - some features may be degraded")
        ...
        try:
            ...
            if model_available:
                ...
                try:
                    ...
                    async with httpx.AsyncClient(timeout=60.0) as client:
                        resp = await client.post(f"{OLLAMA_BASE_URL}/api/generate", json=preload_payload)
            ...
            log_service_status("MODEL", "error", f"Error ensuring default model {DEFAULT_MODEL}: {str(e)}")
